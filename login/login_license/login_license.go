...
func RequestConcurrent(loginId int64, isAdmin bool) error {
	...
	if !config.GetLicenseActive() {
		...
		//FWDO return handler.CreateErrCode("LIC", handler.ErrCodeLicValidityExpired)
		return nil
	}
	...
	if !wasRecent && cnt >= config.GetLicenseLoginCount() {
		...
		return nil
		//FWDO return handler.CreateErrCode("LIC", handler.ErrCodeLicLoginsReached)
	} 
